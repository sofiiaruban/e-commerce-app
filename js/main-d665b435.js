(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const o of i)if(o.type==="childList")for(const p of o.addedNodes)p.tagName==="LINK"&&p.rel==="modulepreload"&&n(p)}).observe(document,{childList:!0,subtree:!0});function s(i){const o={};return i.integrity&&(o.integrity=i.integrity),i.referrerPolicy&&(o.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?o.credentials="include":i.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function n(i){if(i.ep)return;i.ep=!0;const o=s(i);fetch(i.href,o)}})();class H extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render()}render(){this.shadowRoot.innerHTML=`
    <style>
      .products-info {
   ...
   `}}customElements.define("custom-disclosure",H);const O=461,u=1,A=24,l=Math.ceil(O/A),a=5,I=document.querySelector(".products-list"),d=document.querySelector(".pagination"),f=document.querySelector(".cart-list"),L=document.querySelector(".cart-aside"),j=document.querySelector(".cart-icon"),D=document.querySelector(".close-cart"),v=document.querySelector(".main-content"),_=document.querySelector(".total-amount");let r=1;const R="https://cdn.shopify.com/s/files/1/0690/0075/7529/products/AAUvwnj0ICORVuxs41ODOvnhvedArLiSV20df7r8XBjEUQ_s900-c-k-c0x00ffffff-no-rj_72c7d7cb-344c-4f62-ad0d-f75ec755894d.jpg?v=1670516960",c=[];let h=0,y=null;async function F(e){const t=`https://voodoo-sandbox.myshopify.com/products.json?limit=${A}&page=${e}`;try{const s=await fetch(t);if(!s.ok)throw new Error(`Network response was not ok: ${s.status}`);return await s.json()}catch(s){throw s}}function N(e,t){let s="";e.forEach(n=>{const i=n.images[0]&&n.images[0].src||R;s+=`<li class= "flex flex-col w-300" id=${n.id} data-product-id=${n.id}>
    ...
    </li>`}):f.innerHTML+="<p>There are no products yet </p>"}function m(e,t){e.classList.toggle(t)}document.addEventListener("DOMContentLoaded",()=>{x(u),S(u,a),d.addEventListener("click",e=>{if(e.target.classList.contains("pagination-ellipsis-forward")&&P("forward"),e.target.classList.contains("pagination-ellipsis-back")&&P("back"),e.target.classList.contains("page")){const t=parseInt(e.target.textContent,10);t>=u&&t<=l&&(r=t,x(r),q(`${r}`))}}),I.addEventListener("click",e=>{if(e.target.classList.contains("add-button")){const t=e.target.getAttribute("data-product-id");$(t),g(c)}}),f.addEventListener("click",e=>{if(e.target.classList.contains("add-product")){const t=e.target.getAttribute("data-product-id");$(t),g(c)}if(e.target.classList.contains("decrease-product-quantity")){const t=e.target.getAttribute("data-product-id");U(t),g(c)}if(e.target.classList.contains("delete-product")){const t=e.target.getAttribute("data-product-id");M(t),g(c)}}),j.addEventListener("click",e=>{m(L,"hidden"),m(v,"invisible")}),D.addEventListener("click",e=>{m(L,"hidden"),m(v,"invisible")})});
